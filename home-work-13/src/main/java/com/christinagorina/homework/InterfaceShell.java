...
public class InterfaceShell {
    ...
    public BookTo update(@ShellOption({"bookName", "n"}) String bookName,
                         ...
                         @ShellOption({"year", "y"}) String year,
                         @ShellOption({"language", "y"}) String language,
                         @ShellOption({"comment", "a"}) String comment) {
        List<String> comments = Collections.singletonList(comment);
        BookTo updatedBook = new BookTo("1", bookName, genre, year, language, author, comments);
    ...
    public BookTo create(@ShellOption({"bookName", "n"}) String bookName,
                         ...
                         @ShellOption({"year", "y"}) String year,
                         @ShellOption({"language", "y"}) String language,
                         @ShellOption({"comment", "a"}) List<String> comments) {
        BookTo createdBookTo = new BookTo(null, bookName, genre, year, language, author, comments);
        return bookServiceImpl.create(createdBookTo);
    }

    @ShellMethod(key = "getById_book", value = "GetById Book")
    public BookTo getById(@ShellOption({"id", "a"}) String id){
        return bookServiceImpl.findById(id);
    }

    @ShellMethod(key = "delete_book", value = "Deleted Book")
    public void delete(){
        bookServiceImpl.deleteById("3");
    }
