...
public class Util {
    ...
    public static Book createNewFromTo(BookTo bookTo, List<Author> authors, Genre genre) {
        return new Book(bookTo.getId(), bookTo.getName(), genre, bookTo.getOwner(),null, authors);
    }
    ...
    public static BookTo createTo(Book book) {
        if(Objects.nonNull(book)){
            ...
            return new BookTo(book.getId(), book.getName(), authorNames, book.getGenre().getName(), book.getOwner());
        }else{
            return new BookTo();
        }
    }
